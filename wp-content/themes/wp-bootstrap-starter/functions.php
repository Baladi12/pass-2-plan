...
function wp_bootstrap_starter_widgets_init() {
    ...
    register_sidebar( array(
     ...
     register_sidebar( array(
        'name'          => esc_html__( 'Header Cart', 'wp-bootstrap-starter' ),
        'id'            => 'header-cart',
        'description'   => esc_html__( 'Add widgets here.', 'wp-bootstrap-starter' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
...
function custom_field_display_below_title(){
    ...
    // Display ACF text
    if( $text = get_field( 'subtitle', $product->get_id() ) ) {
        echo '<p class="archive-genre">' . $text . '</p>';
    }
    if( $text = get_field( 'date', $product->get_id() ) ) {
        echo '<p class="archive-genre">' . $text . '</p>';
    }
    
    if( $text = get_field( 'Min Time', $product->get_id() ) ) {
        echo '<p class="archive-genre">' . $text . '</p>';
    }
     if( $text = get_field( 'min_time', $product->get_id() ) ) {
        echo '<p class="archive-genre">' . $text . '</p>';
    }
     if( $text = get_field( 'max_time', $product->get_id() ) ) {
        echo '<p class="archive-genre">' . $text . '</p>';
    }
}
...
function getAllMetaData(){
    global $wpdb;
    $results = $wpdb->get_results( "SELECT DISTINCT meta_value FROM {$wpdb->postmeta} WHERE meta_key = 'month_year'", OBJECT );
    return $results;
}
